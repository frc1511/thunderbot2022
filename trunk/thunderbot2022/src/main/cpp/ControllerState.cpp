...
void ControllerState::process()
{
    ...
    { // 14 buttons, 6 axis, 1 pov
        buttons[(2 * i) + 1] = buttons[2 * i];
        if (normalOrRelay)
        { // checks if it should look to the controller for the value of the button
            buttons[2 * i] = myController.GetRawButton(i + 1);
        }
        if (buttons[2 * i] != buttons[(2 * i) + 1] && recordOrNot)
        { // the button changed its state and we are trying to record
        ...
        if (i <= 5)
        { // this will trigger when i is 0-5 so all 6 axes
            if (normalOrRelay)
            { // checks if it should look to the controller for the value of the axes
                axes[2 * i] = myController.GetRawAxis(i);
            }
            if (fabs(axes[2 * i] - axes[(2 * i) + 1]) >= .05 && recordOrNot)
            { // the axis changed by .05
        ...
        { // .GetPOV() is treated as a seventh axis so when i is 6. programmers start at 0 idk why its dumb
            axes[(2 * i) + 1] = axes[2 * i];
            if (normalOrRelay)
            { // checks if it should look to the controller for the value of the pov
                axes[2 * i] = myController.GetPOV();
            }
            if (axes[2 * i] != axes[(2 * i) + 1] && recordOrNot)
            { // the POV changed from what it once was
                std::cout << "which axis" << i << "where?" << axes[2 * i] << "\n";
                recordAxis(i, autoTimer.Get().value(), axes[2 * i]);
            }
        }
    ...
    if (!normalOrRelay)
    { // under autonomous control :D
        if (buttonsDone == false)
        {
            if (buttonsTime.size() > 0)
            { // make sure there is more buttons to change
                if (autoTimer.Get().value() >= buttonsTime[0])
                {                                                                         // the timer has surpassed the next time when somthing is toggled
        ...
        {
            if (axesTime.size() > 0)
            { // make sure there are more axes to be changed
